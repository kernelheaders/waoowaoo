...
const INVALID_LOCATION_KEYWORDS = ['fantasy', 'abstract', 'unspecified', 'anchor point', 'not stated', 'unclear']
...
export function buildCharactersLibInfo(characters: CharacterBrief[]): string {
  if (characters.length === 0) return 'No existing characters'
  return characters
    .map((c, i) => {
      const aliasStr = c.aliases.length > 0 ? `Aliases: ${c.aliases.join(', ')}` : 'Aliases: None'
      const introStr = c.introduction ? `Introduction: ${c.introduction}` : 'Introduction: None'
      return `${i + 1}. ${c.name}\n   ${aliasStr}\n   ${introStr}`
    })
    .join('\n\n')
}
