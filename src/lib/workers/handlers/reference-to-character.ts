...
export async function handleReferenceToCharacterTask(job: Job<TaskJobData>) {
  ...
  if (!imageModel && !extractOnly) {
    throw new Error('Please configure character image model in settings first')
  }
  if (!analysisModel && extractOnly) {
    throw new Error('Please configure analysis model in settings first')
  }
  ...
  await reportTaskProgress(job, 35, {
    ...
    stageLabel: 'Generating character reference sheet',
    displayMode: 'detail',
  })
  ...
  if (isBackgroundJob && appearanceId) {
    ...
    await reportTaskProgress(job, 96, {
      ...
      stageLabel: 'Reference conversion completed',
      displayMode: 'detail',
    })
    return { success: true }
  }
