...
/** Alias matching: split by '/' and check if any alias matches exactly */
...
export async function handleAnalyzeNovelTask(job: Job<TaskJobData>) {
  ...
  await reportTaskProgress(job, 70, {
    ...
    stageLabel: 'Scene analysis completed',
    displayMode: 'detail',
    stepId: 'analyze_locations',
    stepTitle: 'Scene analysis',
    stepIndex: 2,
  ...
  await reportTaskProgress(job, 80, {
    ...
    stageLabel: 'Prop analysis completed',
    displayMode: 'detail',
    stepId: 'analyze_props',
    stepTitle: 'Prop analysis',
    stepIndex: 3,
  ...
  for (const item of parsedLocations) {
    ...
    const invalidKeywords = ['fantasy', 'abstract', 'unspecified', 'anchor point', 'not stated', 'unclear']
